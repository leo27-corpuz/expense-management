<template>
    <article>
        ...
        <template v-else>
            <div class="back">
                <router-link :to="{ name: 'expenseuser' }"><button><i class="fa-solid fa-angle-left"></i>
...
export default{
    ...
    created(){
        ...
        Promise.all([promisegetCategoryData, promiseGetExpenseData]).then(() => {
            setTimeout(() => {
    ...
    methods: {
        async submit(){
            ...
            try {
                ...
                const response = await axiosIntance.post(`/api/user/expense/update/${this.$route.params.id}`, formData, config)
                ...
                if (res) {
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            this.loadingMessage = ''
                            this.error = []
                            this.$router.push({name: 'expenseuser'})
        ...
        async getCategoryData(){
            try {
                ...
                const response = await axiosIntance.get('/api/user/category', config)
        ...
        async getExpenseData(){
            try {
                ...
                const response = await axiosIntance.get(`/api/user/expense/update/${this.$route.params.id}`, config)
                const res = await response.data
                console.log(res)
