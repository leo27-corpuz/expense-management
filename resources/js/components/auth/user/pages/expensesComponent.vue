<template>
    <article>
        ...
        <template v-else>
            ...
            <div class="add-data">
                <router-link :to="{ name: 'expenseusercreate' }"><button>Add Expense <i
    ...
    methods: {
        async getExpensesData(){
            try {
                ...
                const response = await axiosIntance.get('/api/user/expense', config)
        ...
        async deleteProcess(){
            ...
            try {
                ...
                const response = await axiosIntance.delete(`/api/user/expense/delete/${this.deletion.idValueToDelete}`, config)
                const res = await response.data
                if (res) {
                    setTimeout(() => {
                        this.statusSubmit = !this.statusSubmit
        ...
        updateData(id){
            this.$router.push({name: 'expensesuserupdate', params: {id: id}})
        }
    }
}
</script>
